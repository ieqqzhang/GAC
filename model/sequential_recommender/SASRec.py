...
class SASRec(SeqAbstractRecommender):
    ...
    def _create_variable(self):
        ...
        item_embeddings = tf.get_variable('item_embeddings', dtype=tf.float32,
                                          shape=[self.items_num, self.embedding_size],
                                          regularizer=l2_regularizer)

        zero_pad = tf.zeros([1, self.embedding_size], name="padding")
        item_embeddings = tf.concat([item_embeddings, zero_pad], axis=0)
        self.item_embeddings = item_embeddings * (self.embedding_size ** 0.5)

        self.position_embeddings = tf.get_variable('position_embeddings', dtype=tf.float32,
                                                   shape=[self.max_len, self.embedding_size],
                                                   regularizer=l2_regularizer)
    ...
    def build_graph(self):
        ...
        seq_emb = tf.reshape(self.seq, [tf.shape(self.item_seq_ph)[0] * self.max_len, self.embedding_size])  # (b*l, d)
    ...
    def train_model(self):
        ...
        for epoch in range(self.epochs):
            item_seq_list, item_pos_list, item_neg_list = self.get_train_data()     # , user_list
            data = DataIterator(item_seq_list, item_pos_list, item_neg_list,    # user_list,
